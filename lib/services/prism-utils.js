...
function PrismUtils() {
  ...
  this.isValidValue = function(value) {
    ...
    return (_.isRegExp(value) || _.isArray(value) && !_.isEmpty(value) || _.isBoolean(value) && value) || _.isFunction(value);
  };

  this.filterUrl = function(config, url) {
    if (!this.isValidValue(config.ignoreParameters) && !config.rewritePath) {
      return url;
    }
    ...
    var rewrite = config.rewritePath;

    var slicedUrl = url.split(/\?|\&/);

    var urlPath = slicedUrl[0];
    if(typeof rewrite !== 'undefined' && _.isFunction(rewrite)) {
        urlPath = rewrite(urlPath);
    }
